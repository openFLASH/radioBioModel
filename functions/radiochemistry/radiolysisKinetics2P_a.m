...
% * |param.kb8| -_SCALAR_- [OPTIONAL, default = 0.0408 s-1] Rate constant for decay of LOOr
%
...
% [20] Babbs, C. F. & Steiner, M. G. SIMULATION OF FREE RADICAL REACTIONS IN BIOLOGY AND MEDICINE : A NEW TWO-COMPARTMENT KINETIC MODEL OF. Free Radic. Biol. Med. 8, 471–485 (1990).
% [21] Spector, W. S., ed. Handbook of biological data. Philadelphia: W. B. Saunders Company; 1956:70-77
%
%% Contributors
...
%Compute the multiplicative coefficient to obtain the effective rate constant
% depnding on which phase the reactants A and B are located
% The reactant is either totally in water (P=1) or totally in lipids (P=2)
% or with same concentration in both pahses (P=3)
% According to [20] , table 4
%
% NB: This modification to the original code has the effect of reducing the peak concentration of [ROO*] by a fator 35
%
% ke (Pa , Pb) : multiplicative coefficient of the dose rate when first reactant is in phase Pa and second reactant in phase Pb
% |Pa| -_INTEGER_- Phase of reactant A See below for the definition of P
% |Pb| -_INTEGER_- Phase of reactant B
%-------------------------------------
r = 40;% r = Vw + Vl / Vl where Vw is the water volume and Vl is the lipid volume in cell
        %Value according to [20] who is referencing [21]

%Table 4 in [20]
keq = ones(3,3);
keq(1,1) = r ./ (r-1); %water - water
keq(1,2) = 0; %water - lipid
keq(2,1) = 0; %lipid - water
keq(2,2) = r ; %Lipid - lipid
...
d_Lr      = 8; %Alkyl radicals
d_LOOr    = 9; %peroxylalkyl radicals
d_LOOH    = 10;
...
%Define the phase in which the reactant is located
% P = 1 Water
% P= 2 Lipid
...
P(d_HO2m)  = 1; %This is  charged -> in water
P(d_OHr)   = 3; %In both phases, as in [20]
P(d_Orm)   = 1; %This is charged -> in water
P(d_Hr)    = 3; %Neutral radical -> in both phases
P(d_H2)    = 3; %Neutral gas. Can diffuse in both phases
P(d_HO2r)  = 3; %neutral radical, polar molecule -> in both pahses
P(d_O2rm)  = 1; %This is charged -> in water
P(d_Lr)    = 2; %Long alkyl chain and neutral -> in lipid
P(d_LOOr)  = 2; %Long alkyl chain and neutral -> in lipid
P(d_water) = 1; %Water is only in water
P(d_LH)    = 2; %Long alkyl chain and neutral -> in lipid
P(d_Hp)    = 1; %This is charged -> in water
P(d_OHm)   = 1; %This is charged -> in water
...
Lr   = C(d_Lr); %Alkyl radicals
LOOr = C(d_LOOr); %peroxylRadical radicals
LOOH = C(d_LOOH); %peroxylRadical radicals
...
R1        = keq(P(d_e) , P(d_water) ) .* 19      .* e .* water;
d_R1(d_e) = keq(P(d_e) , P(d_water) ) .* 19           .* water; % d_R1(d_e) = the derivative of the reaction rate R1 with respect to species e
...
R2        = keq(P(d_e) , P(d_e) ) .* 1.1e10 /2  .* e .* e;
d_R2(d_e) = keq(P(d_e) , P(d_e) ) .* 1.1e10 /2  .* 2 .* e;

R3        = keq(P(d_e) , P(d_Hr) ) .* 2.5e10  .* e .* Hr;
d_R3(d_e) = keq(P(d_e) , P(d_Hr) ) .* 2.5e10       .* Hr;
d_R3(d_Hr)= keq(P(d_e) , P(d_Hr) ) .* 2.5e10  .* e;

R4           = keq(P(d_e) , P(d_OHr) ) .* 3e10    .* e .* OHr;
d_R4(d_e)    = keq(P(d_e) , P(d_OHr) ) .* 3e10         .* OHr;
d_R4(d_CtOHr)= keq(P(d_e) , P(d_OHr) ) .* 3e10    .* e        .* dOHr_dCt; %d_R4/dCt = (d_R4/dOHr) * (dOHr/dCt)

R5           = keq(P(d_e) , P(d_Orm) ) .* 2.2e10  .* e .* Orm;
d_R5(d_e)    = keq(P(d_e) , P(d_Orm) ) .* 2.2e10       .* Orm;
d_R5(d_CtOHr)= keq(P(d_e) , P(d_Orm) ) .* 2.2e10  .* e         .* dOrm_dCt;

R6        = keq(P(d_e) , P(d_Hp) ) .* 2.3e10  .* e .* Hp;
d_R6(d_e) = keq(P(d_e) , P(d_Hp) ) .* 2.3e10       .* Hp;

R7            = keq(P(d_e) , P(d_H2O2) ) .* 1.1e10  .* e .* H2O2;
d_R7(d_e)     = keq(P(d_e) , P(d_H2O2) ) .* 1.1e10       .* H2O2;
d_R7(d_CtH2O2)= keq(P(d_e) , P(d_H2O2) ) .* 1.1e10  .* e         .* dHO2m_dCt;

R8            = keq(P(d_e) , P(d_HO2m) ) .* 3.5e9   .* e .* HO2m;
d_R8(d_e)     = keq(P(d_e) , P(d_HO2m) ) .* 3.5e9        .* HO2m;
d_R8(d_CtH2O2)= keq(P(d_e) , P(d_HO2m) ) .* 3.5e9   .* e        .* dHO2m_dCt;

R9        = keq(P(d_e) , P(d_O2) ) .* 1.9e10  .* e .* O2;
d_R9(d_e) = keq(P(d_e) , P(d_O2) ) .* 1.9e10       .* O2;
d_R9(d_O2)= keq(P(d_e) , P(d_O2) ) .* 1.9e10  .* e;

R10            = keq(P(d_e) , P(d_O2rm) ) .* 1.3e10 .* e .* O2rm;
d_R10(d_e)     = keq(P(d_e) , P(d_O2rm) ) .* 1.3e10      .* O2rm;
d_R10(d_CtH02r)= keq(P(d_e) , P(d_O2rm) ) .* 1.3e10 .* e        .* dO2rm_dCt;


%REaction with radical H.
R11        = keq(P(d_Hr) , P(d_water) ) .* 10  .* Hr .* water;
d_R11(d_Hr)= keq(P(d_Hr) , P(d_water) ) .* 10        .* water;

R12        = keq(P(d_Hr) , P(d_Hr) ) .* 1.55e10 /2 .* Hr .* Hr;
d_R12(d_Hr)= keq(P(d_Hr) , P(d_Hr) ) .* 1.55e10 /2 .* 2  .* Hr;

R13           = keq(P(d_Hr) , P(d_OHr) ) .* 7e9     .* Hr .* OHr;
d_R13(d_Hr)   = keq(P(d_Hr) , P(d_OHr) ) .* 7e9           .* OHr;
d_R13(d_CtOHr)= keq(P(d_Hr) , P(d_OHr) ) .* 7e9     .* Hr       .* dOHr_dCt;

R14         = keq(P(d_Hr) , P(d_OHm) ) .* 2.2e7   .* Hr .* OHm;
d_R14(d_Hr) = keq(P(d_Hr) , P(d_OHm) ) .* 2.2e7         .* OHm;

R15            = keq(P(d_Hr) , P(d_H2O2) ) .* 9e7     .* Hr .* H2O2;
d_R15(d_Hr)    = keq(P(d_Hr) , P(d_H2O2) ) .* 9e7           .* H2O2;
...
d_R17(d_CtH02r)= keq(P(d_Hr) , P(d_HO2r) ) .* 1e10    .* Hr        .* dHO2r_dCt;

%REaction with radical OH.
R18            = keq(P(d_OHr) , P(d_OHr) ) .* 1.1e10 /2  .* OHr .* OHr;
d_R18(d_CtOHr) = keq(P(d_OHr) , P(d_OHr) ) .* 1.1e10 /2  .* 2   .* OHr .* dOHr_dCt;

R20           = keq(P(d_OHr) , P(d_H2) ) .* 4.2e7    .* OHr .* H2;
d_R20(d_CtOHr)= keq(P(d_OHr) , P(d_H2) ) .* 4.2e7           .* H2 .* dOHr_dCt;
d_R20(d_H2)   = keq(P(d_OHr) , P(d_H2) ) .* 4.2e7    .* OHr;

% R21           = keq(P(d_OHr) , P(d_OHm) ) .* 1.3e10   .* OHr .* OHm;  %This cancels out in the additions. No need to compute it.
% d_R21(d_CtOHr)= keq(P(d_OHr) , P(d_OHm) ) .* 1.3e10          .* OHm  .* dOHr_dCt;

R22            = keq(P(d_OHr) , P(d_H2O2) ) .* 2.7e7    .* OHr .* H2O2;
d_R22(d_CtOHr) = keq(P(d_OHr) , P(d_H2O2) ) .* 2.7e7           .* H2O2 .* dOHr_dCt;
d_R22(d_CtH2O2)= keq(P(d_OHr) , P(d_H2O2) ) .* 2.7e7    .* OHr         .* dH2O2_dCt;

R23            = keq(P(d_OHr) , P(d_HO2m) ) .* 7.5e9    .* OHr .* HO2m;
d_R23(d_CtOHr) = keq(P(d_OHr) , P(d_HO2m) ) .* 7.5e9           .* HO2m .* dOHr_dCt;
d_R23(d_CtH2O2)= keq(P(d_OHr) , P(d_HO2m) ) .* 7.5e9    .* OHr         .* dHO2m_dCt;

R25            = keq(P(d_OHr) , P(d_HO2r) ) .* 6e9      .* OHr .* HO2r;
d_R25(d_CtOHr) = keq(P(d_OHr) , P(d_HO2r) ) .* 6e9             .* HO2r .* dOHr_dCt;
d_R25(d_CtH02r)= keq(P(d_OHr) , P(d_HO2r) ) .* 6e9      .* OHr         .* dHO2r_dCt;

R26            = keq(P(d_OHr) , P(d_O2rm) ) .* 8e9      .* OHr .* O2rm;
d_R26(d_CtOHr) = keq(P(d_OHr) , P(d_O2rm) ) .* 8e9             .* O2rm .* dOHr_dCt;
d_R26(d_CtH02r)= keq(P(d_OHr) , P(d_O2rm) ) .* 8e9      .* OHr         .* dO2rm_dCt;

RdOH           = keq(P(d_OHr) , P(d_water) ) .* param.kdOH .* OHr; % Reaction of HO^. with thiols. Thiols are in water
d_RdOH(d_CtOHr)= keq(P(d_OHr) , P(d_water) ) .* param.kdOH       .* dOHr_dCt;

% %REaction with radical O.-
% R27           =  keq(P(d_Orm) , P(d_water) ) .* 1.8e6  .* Orm .* water;  %This cancels out in the additions. Nio need to compute
% d_R27(d_CtOHr)=  keq(P(d_Orm) , P(d_water) ) .* 1.8e6         .* water .* dOrm_dCt;

R32            = keq(P(d_Orm) , P(d_O2) ) .* 3.6e9   .* Orm .* O2;
d_R32(d_CtOHr) = keq(P(d_Orm) , P(d_O2) ) .* 3.6e9          .* O2 .* dOrm_dCt;
d_R32(d_O2)    = keq(P(d_Orm) , P(d_O2) ) .* 3.6e9   .* Orm;

R33            = keq(P(d_Orm) , P(d_O2rm) ) .* 6e8     .* Orm .* O2rm;
d_R33(d_CtOHr) = keq(P(d_Orm) , P(d_O2rm) ) .* 6e8            .* O2rm .* dOrm_dCt;
d_R33(d_CtH02r)= keq(P(d_Orm) , P(d_O2rm) ) .* 6e8     .* Orm         .* dO2rm_dCt;
...
%Formation and decay of alkyl radicals Lr
% OHr + LH -> H2O + Lr
% Lr + O2 -> LOOr
% NB: LOOr = peroxyl radical
...
%L + e -> L*
Rde       = keq(P(d_e) , P(d_LH) ) .* param.kde .* LH .* e; % Reaction of electron with bioloigical molecules
...
%L + H* -> L* + H2
RdH        = keq(P(d_Hr) , P(d_LH) ) .* param.kdH .* LH .* Hr; % Reaction of H^. with biological molecules
d_RdH(d_Hr)= keq(P(d_Hr) , P(d_LH) ) .* param.kdH .* LH;
...
%//////////
% NB: Now, LH means 'lipids'. It is therefore not necessary to introduce a separate [lipid] concentraiton
% We use [LH] instead
%//////////
%LH + LOO* -> L* + LOOH
...
if(isfield(param,'Fe2p'))
  [Rb5 , Rb7 , d_Rb5_dH2O2 , dRb7] = ironRadicalsKinetics(H2O2, O2, LH,keq(P() , P() ) .* param.Fe2p);
  d_Rb5(d_CtH2O2) = d_Rb5_dH2O2 .* dH2O2_dCt;
else
  [Rb5 , Rb7 , d_Rb5_dH2O2 , dRb7] = ironRadicalsKinetics(H2O2, O2, LH);
  d_Rb5(d_CtH2O2) = d_Rb5_dH2O2 .* dH2O2_dCt;
end
...
% Ct = [H2O2] + [HO2m]
dydt(d_CtH2O2) = -R7   -R8   -R15   +R17   +R18   -R22   -R23   +Rb1   -Rb4   -Rb5   -Rb9   -2.*Rb10  + param.krH2O2 .* param.R(t,param);
Jac(d_CtH2O2,:)= -d_R7 -d_R8 -d_R15 +d_R17 +d_R18 -d_R22 -d_R23 +d_Rb1 -d_Rb4 -d_Rb5 -d_Rb9 -2.*d_Rb10;

% Ct = [OHr] + [Orm]
dydt(d_CtOHr)  = -R4   -R5   +R7   +R8   +R11   -R13   +R15   -2.*R18   -R20   -R22   -R23   -R25   -R26   -R32   -R33   -Rb2   +Rb5   -RdOH   + param.krOHr .* param.R(t,param);
Jac(d_CtOHr,:) = -d_R4 -d_R5 +d_R7 +d_R8 +d_R11 -d_R13 +d_R15 -2.*d_R18 -d_R20 -d_R22 -d_R23 -d_R25 -d_R26 -d_R32 -d_R33 -d_Rb2 +d_Rb5 -d_RdOH;
...
% Ct = [HO2r] + [O2rm]
dydt(d_CtH02r)   = R9   -R10   +R16   -R17   +R23   -R25    -R26    -R33  -2.* Rb1   -Rb4 ;
Jac(d_CtH02r,:)  = d_R9 -d_R10 +d_R16 -d_R17 +d_R23 -d_R25 -d_R26  -d_R33 -2.* d_Rb1 -d_Rb4;

dydt(d_Lr)   = Rb2   +Rb7   -Rb3   -Rbr     + Rb11 -2 .* Rbr2 + param.krR .* param.R(t,param); %dLr_dt
Jac(d_Lr,:)  = d_Rb2 +d_Rb7 -d_Rb3 -d_Rbr + d_Rb11 -2 .* d_Rbr2;

dydt(d_LOOr)   = Rb3   -Rb8   -Rb11   -2 .* Rb6; %dLOOr_dt [12] for 1st order decay
Jac(d_LOOr,:)  = d_Rb3 -d_Rb8 -d_Rb11 -2 .* d_Rb6;

dydt(d_LOOH)   = Rb11    + Rb8; % Creation of lipid degradation products
Jac(d_LOOH,:)  = d_Rb11  + d_Rb8;
